...
DROP VIEW IF EXISTS care_overview_view;
DROP TABLE IF EXISTS quick_access_items;
...
CREATE TABLE appointments (
    ...
    visit_type TEXT NOT NULL CHECK (visit_type IN ('in_person', 'telehealth')),
    location TEXT,
    reason TEXT NOT NULL,
    status TEXT NOT NULL DEFAULT 'scheduled'
        CHECK (status IN ('scheduled', 'completed', 'cancelled', 'missed')),
    check_in_status TEXT NOT NULL DEFAULT 'not_started'
        CHECK (check_in_status IN ('not_started', 'available', 'complete', 'not_required')),
    prep_notes TEXT,
    created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE TABLE follow_up_tasks (
    ...
    task_type TEXT NOT NULL
        CHECK (task_type IN ('schedule', 'lab', 'prescription', 'review_result', 'referral', 'message', 'form')),
    due_at TEXT,
    status TEXT NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'complete', 'dismissed')),
...
CREATE TABLE prescriptions (
    ...
    last_filled_at TEXT,
    next_refill_at TEXT,
    status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'refill_due', 'expired'))
);
...
CREATE TABLE referrals (
    ...
    status TEXT NOT NULL DEFAULT 'pending'
        CHECK (status IN ('pending', 'scheduled', 'complete', 'expired')),
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
    expires_at TEXT
);
...
CREATE TABLE notifications (
    ...
    source_type TEXT NOT NULL
        CHECK (source_type IN ('message', 'test_result', 'task', 'referral', 'prescription', 'appointment')),
...
SELECT
    ...
    (SELECT a.id FROM appointments a
        ...
        ORDER BY a.starts_at DESC LIMIT 1) AS last_completed_appointment_id,
    (SELECT MAX(a.starts_at) FROM appointments a
        WHERE a.patient_id = p.id AND a.status = 'completed') AS last_visit_at,
    (SELECT a.id FROM appointments a
        WHERE a.patient_id = p.id AND a.status = 'scheduled' AND a.starts_at > datetime('now')
        ORDER BY a.starts_at LIMIT 1) AS next_appointment_id,
    (SELECT MIN(a.starts_at) FROM appointments a
        WHERE a.patient_id = p.id AND a.status = 'scheduled' AND a.starts_at > datetime('now')) AS next_appointment_at,
    (SELECT COUNT(*) FROM follow_up_tasks t
        WHERE t.patient_id = p.id AND t.status = 'open') AS open_task_count,
    (SELECT COUNT(*) FROM follow_up_tasks t
        WHERE t.patient_id = p.id AND t.status = 'open' AND t.due_at < datetime('now')) AS overdue_task_count,
    (SELECT COUNT(*) FROM messages m
        WHERE m.patient_id = p.id AND m.status = 'unread') AS unread_message_count,
    (SELECT COUNT(*) FROM test_results r
        WHERE r.patient_id = p.id AND r.status = 'new') AS new_result_count,
    (SELECT COUNT(*) FROM referrals rf
        WHERE rf.patient_id = p.id AND rf.status IN ('pending', 'scheduled')) AS active_referral_count,
    (SELECT COUNT(*) FROM prescriptions rx
        JOIN medications md ON md.id = rx.medication_id
        WHERE md.patient_id = p.id AND md.status = 'active' AND rx.status = 'refill_due') AS refill_due_count,
    (SELECT COUNT(*) FROM notifications n
        WHERE n.patient_id = p.id AND n.status = 'open') AS open_notification_count
FROM patients p;
